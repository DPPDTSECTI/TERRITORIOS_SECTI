...
const CustomTooltip = ({
 ...
 className="bg-surface border border-white/40 shadow-card-soft shadow-primary-900/10 rounded-xl p-5 w-[320px] sm:w-[380px] relative z-50 flex flex-col font-sans "
 >
 <div className="flex justify-between items-center mb-3">
 {step.title ? (
 <h3 className="text-[16px] font-medium text-text-primary tracking-tight">{step.title}</h3>
 ) : (
 ...
 <span className="text-[11px] font-medium text-primary-600 uppercase ">Passo {index + 1} de {size}</span>
 </div>
 )}
 
 <button {...closeProps} title="Fechar Tutorial" className="text-text-muted hover:text-text-primary hover:bg-surface-soft p-1.5 rounded-lg transition-colors">
 ...
 {!isLastStep && (
 <button {...skipProps} className="text-[12px] font-medium text-text-muted hover:text-text-primary transition-colors bg-transparent border-none cursor-pointer">
 Pular Tutorial
 </button>
 )}
 {index > 0 && (
 <button {...backProps} className="text-[12px] font-medium text-primary-600 hover:text-primary-800 transition-colors bg-transparent border-none cursor-pointer">
 Voltar
 </button>
 )}
 {continuous && !isLastStep && (
 <button {...primaryProps} className="px-4 py-2 text-[13px] font-medium bg-primary-600 text-white rounded-lg shadow-md shadow-primary-600/30 hover:bg-primary-700 hover:shadow-lg transition-all cursor-pointer">
 Próximo
 </button>
 )}
 {(!continuous || isLastStep) && (
 <button {...primaryProps} className="px-4 py-2 text-[13px] font-medium bg-primary-900 text-white rounded-lg shadow-md shadow-primary-900/30 hover:bg-primary-600 hover:shadow-lg transition-all cursor-pointer">
 Concluir
 </button>
 )}
 </div>

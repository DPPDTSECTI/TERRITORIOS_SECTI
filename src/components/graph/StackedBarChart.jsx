      ...
      <div className="flex items-center flex-wrap gap-x-2.5 gap-y-1 my-1 text-[9px] font-semibold text-text-secondary shrink-0">
        {categories.map((cat) => (
          <div key={cat.key} className="flex items-center gap-1">
            ...
            <span className="truncate max-w-[110px] text-text-primary font-bold">{cat.shortLabel || cat.label}</span>
          </div>
        ))}
      </div>
              ...
              return (
                ...
                >
                  ...
                  {hoveredSlice && hoveredSlice.label === item.label && !hoveredTotalTerritory && (
                    <div
                      className={`absolute bottom-full mb-2 bg-surface text-text-primary shadow-sm rounded-lg px-2.5 py-1.5 pointer-events-none z-[100] whitespace-nowrap text-left flex flex-col gap-1 border border-border min-w-[140px] max-w-[190px] transition-all duration-150 animate-in fade-in zoom-in-95 transform-gpu ${isFirst ? 'left-0' : isLast ? 'right-0' : 'left-1/2 -translate-x-1/2'
                        }`}
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[9px] font-medium text-text-muted bg-surface-soft px-1.5 py-0.2 rounded-full shrink-0">
                      ...
                      <div className="flex items-center justify-between gap-2 pt-0.5 border-t border-border">
                        ...
                        <span className="text-[10px] font-bold text-text-primary px-1.5 py-0.2 rounded bg-surface-soft shrink-0">
                          {hoveredSlice.count}
                        </span>
                      </div>
                  ...
                  {hoveredTotalTerritory && hoveredTotalTerritory.label === item.label && (
                    <div
                      className={`absolute bottom-full mb-2 bg-surface text-text-primary shadow-sm rounded-xl p-2.5 pointer-events-none z-[100] whitespace-nowrap text-left flex flex-col gap-1.5 border border-border min-w-[165px] max-w-[210px] transition-all duration-150 animate-in fade-in zoom-in-95 transform-gpu ${isFirst ? 'left-0' : isLast ? 'right-0' : 'left-1/2 -translate-x-1/2'
                        }`}
                    >
                      <div className="flex items-center justify-between border-b border-border pb-1 gap-2">
                        <span className="text-[11px] font-bold text-text-primary truncate max-w-[115px]">
                  ...
                  <div
                    ...
                    className="flex items-center justify-center min-w-[18px] px-1 py-0.2 rounded-full bg-surface-soft border border-border text-[8.5px] font-bold text-text-secondary group-hover/bar:bg-primary-900 group-hover/bar:text-white group-hover/bar:scale-110 transition-all mb-1 select-none cursor-pointer shadow-2xs"
                  ...
                  <div
                    ...
                    title={item.label}
                  >
                    <span className="text-[7.5px] sm:text-[8px] font-bold text-text-muted group-hover/bar:text-text-primary transition-colors whitespace-pre-line line-clamp-2 break-words">
                      {formattedName}
                    </span>
                  </div>
                </div>

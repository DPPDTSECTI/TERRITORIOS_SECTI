...
function StackedBarChart({
  ...
  return (
    <div className="bg-white rounded-[24px] border border-transparent hover:border-[#D6EAF8]/50 shadow-[0_4px_24px_rgba(29,53,87,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(29,53,87,0.1)] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] p-5 relative flex flex-col justify-start h-full group cursor-default min-h-0">
        ...
        {allowToggleView && (
          <div className="flex items-center bg-[#F1F5F9] p-0.5 rounded-full border border-[#E2E8F0]">
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-bold rounded-full transition-all flex items-center gap-1 cursor-pointer ${minimizedFilter === 'acima'
            ...
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-bold rounded-full transition-all flex items-center gap-1 cursor-pointer ${minimizedFilter === 'abaixo'
      ...
      <div className="flex items-center flex-wrap gap-x-2.5 gap-y-1 my-1 text-[9px] font-semibold text-[#457B9D] shrink-0">
        {categories.map((cat) => (
          <div key={cat.key} className="flex items-center gap-1">
            ...
            <span className="truncate max-w-[110px] text-[#1D3557] font-bold">{cat.shortLabel || cat.label}</span>
          </div>
        ))}
      </div>
              ...
              return (
                ...
                >
                  ...
                  {hoveredSlice && hoveredSlice.label === item.label && !hoveredTotalTerritory && (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[10px] font-extrabold text-white truncate max-w-[100px]">
                          {hoveredSlice.label}
                        </span>
                        <span className="text-[9px] font-bold text-[#A8DADC] bg-white/15 px-1.5 py-0.2 rounded-full shrink-0">
                      ...
                      <div className="flex items-center justify-between gap-2 pt-0.5 border-t border-white/10">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="w-2 h-2 rounded-full shrink-0 border border-white/50" style={{ backgroundColor: hoveredSlice.colorHex }}></span>
                          <span className="text-[9.5px] font-medium text-white/90 truncate">
                            {hoveredSlice.segmentName}
                          </span>
                        </div>
                        <span className="text-[10px] font-black text-white px-1.5 py-0.2 rounded bg-white/20 shrink-0">
                          {hoveredSlice.count}
                        </span>
                      </div>

                      <div className={`absolute -bottom-1 w-2 h-2 bg-[#1D3557] rotate-45 border-r border-b border-white/20 ${isFirst ? 'left-3' : isLast ? 'right-3' : 'left-1/2 -translate-x-1/2'
                        }`}></div>
                    </div>
                  )}
                  ...
                  {hoveredTotalTerritory && hoveredTotalTerritory.label === item.label && (
                    ...
                    >
                      <div className="flex items-center justify-between border-b border-white/15 pb-1 gap-2">
                        <span className="text-[11px] font-extrabold text-white truncate max-w-[115px]">
                          {hoveredTotalTerritory.label}
                        </span>
                        <span className="text-[9px] font-bold text-[#A8DADC] bg-white/15 px-2 py-0.5 rounded-full shrink-0">
                          {hoveredTotalTerritory.total} ativos
                        </span>
                      </div>

                      <div className="flex flex-col gap-1 text-[9px] text-white/80 max-h-[140px] overflow-hidden">
                        {categories.filter(c => Number(item.segments?.[c.key] || 0) > 0).map(c => (
                          <div key={c.key} className="flex items-center justify-between gap-2">
                            ...
                            <span className="font-bold text-white bg-white/10 px-1 rounded">{item.segments[c.key]}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div
                    ...
                    className="flex items-center justify-center min-w-[18px] px-1 py-0.2 rounded-full bg-[#F1F5F9] border border-[#D6EAF8] text-[8.5px] font-extrabold text-[#1D3557] group-hover/bar:bg-[#2563EB] group-hover/bar:text-white group-hover/bar:scale-110 transition-all mb-1 select-none cursor-pointer shadow-2xs"
                  ...
                  <div className="w-full flex items-end justify-center h-full relative">
                    <div
                      className="w-full max-w-[15px] sm:max-w-[18px] rounded-t-[4px] rounded-b-[2px] relative flex flex-col-reverse overflow-hidden transition-all duration-300 ease-out group-hover/bar:scale-105 group-hover/bar:ring-2 group-hover/bar:ring-[#2563EB]/40 shadow-xs border border-black/5"
                  ...
                  <div
                    ...
                    title={item.label}
                  >
                    <span className="text-[7.5px] sm:text-[8px] font-bold text-[#64748B] group-hover/bar:text-[#1D3557] transition-colors whitespace-pre-line line-clamp-2 break-words">
                      {formattedName}
                    </span>
                  </div>
                </div>

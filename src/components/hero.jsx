...
export default function LandingHero() {
  ...
  return (
    <main className="w-full min-h-screen font-sans relative text-text-primary scroll-smooth bg-background overflow-x-hidden transition-colors duration-300">

      {/* ================= FUNDO SUTIL E LIMPO (SEM BLOBS QUE PREJUDIQUEM A LEITURA) ================= */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-[10%] -left-[10%] w-[500px] h-[500px] lg:w-[800px] lg:h-[800px] bg-primary-500/10 dark:bg-primary-600/15 rounded-full filter blur-[140px] opacity-40"></div>
        <div className="absolute top-[20%] right-[-10%] w-[400px] h-[400px] lg:w-[600px] lg:h-[600px] bg-primary-400/10 dark:bg-primary-500/10 rounded-full filter blur-[140px] opacity-30"></div>
        <div className="absolute -bottom-[20%] left-[15%] w-[500px] h-[500px] lg:w-[900px] lg:h-[900px] bg-primary-600/5 dark:bg-primary-400/10 rounded-full filter blur-[160px] opacity-30"></div>

        {/* Linhas orbitais arquitetônicas sutis */}
        <div className="absolute top-[-5%] right-[-40%] lg:right-[-25%] w-[1200px] h-[1200px] lg:w-[1800px] lg:h-[1800px] border border-primary-900/[0.05] dark:border-primary-400/[0.08] rounded-[40%] rotate-[35deg]" />
        <div className="absolute top-[15%] right-[-30%] lg:right-[-15%] w-[1300px] h-[1300px] lg:w-[2000px] lg:h-[2000px] border border-primary-900/[0.04] dark:border-primary-400/[0.06] rounded-[35%] rotate-[60deg]" />

        {/* Marca d'água Territorial (Bahia Silhouette) */}
        <div className="absolute top-1/2 right-0 -translate-y-1/2 translate-x-[50%] sm:translate-x-[45%] md:translate-x-[40%] lg:translate-x-[40%] xl:translate-x-[40%] w-[800px] sm:w-[900px] lg:w-[1100px] xl:w-[1250px] pointer-events-none mix-blend-multiply dark:mix-blend-screen flex items-center justify-center">
          <style>{`
          ...
          `}</style>
          <svg viewBox="0 0 800 845" fill="none" stroke="currentColor" strokeWidth="1" className="w-full h-auto text-primary-900 dark:text-primary-300">
      ...
      <section className="relative w-full min-h-[calc(100vh-22px)] flex flex-col justify-between px-6 sm:px-10 lg:px-12 pt-6 sm:pt-8 pb-12 sm:pb-16 z-10 max-w-[1600px] mx-auto">
        ...
        {/* HEADER SUPERIOR */}
        <header className="w-full flex items-center justify-between pb-4 sm:pb-5 border-b border-border">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-2.5 sm:gap-3">
              ...
              />
              <span className="font-sans font-bold text-sm sm:text-base tracking-wider uppercase text-text-primary">
                Painel Territorial CT&I
              </span>
            </div>

            <span className="text-border-strong font-light text-xl select-none">|</span>

            <img
              src={isDark ? '/img/Brasao-Horizontal_Branco.webp' : '/img/brasao_preto.webp'}
              alt="Governo do Estado da Bahia"
              className="h-10 sm:h-12 w-auto object-contain shrink-0 transition-opacity duration-300"
            />
          </div>

          <div className="flex items-center gap-4 sm:gap-6">
            <nav className="flex items-center gap-5 sm:gap-8 text-xs sm:text-sm font-semibold text-text-secondary">
              <Link to="/" className="text-primary-600 dark:text-primary-400 font-bold transition-colors">
                Início
              </Link>
              <Link to="/territorios" className="hover:text-text-primary transition-colors">
                Dashboard
              </Link>
              <Link to="/sobre" className="hover:text-text-primary transition-colors">
                Sobre
              </Link>
              <Link to="/relatorio" className="hover:text-text-primary transition-colors">
                Relatório
              </Link>
            </nav>
        ...
        <div className="w-full flex-1 flex flex-col justify-center my-auto py-8 sm:py-14">
          <div className="max-w-4xl lg:max-w-5xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-surface border border-primary-300/40 dark:border-primary-700/50 text-xs font-bold uppercase text-primary-700 dark:text-primary-300 mb-4 sm:mb-6 shadow-xs justify-center leading-none">
            ...
            <h1 className="font-sans font-extrabold text-4xl sm:text-5xl lg:text-[68px] leading-[1.08] mb-4 sm:mb-6 text-text-primary tracking-tight text-left">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-text-secondary font-medium leading-relaxed mb-6 sm:mb-8 min-h-[50px] max-w-3xl text-left">
              {typedText}
              <span className="inline-block w-1.5 h-4 ml-1 bg-primary-600 dark:bg-primary-400 animate-pulse align-middle" />
            </p>

            <div className="flex flex-wrap items-center gap-3.5">
              <button
                onClick={() => navigate('/territorios')}
                className="px-7 py-3.5 rounded-xl bg-primary-700 hover:bg-primary-800 dark:bg-primary-600 dark:hover:bg-primary-500 flex items-center gap-3 shadow-md hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 text-white w-fit cursor-pointer group"
              >
              ...
              <a
                href="#modulos"
                className="px-5 py-3.5 rounded-xl bg-surface border border-border text-text-primary font-semibold text-sm hover:bg-surface-soft hover:border-border-strong transition-all inline-flex items-center gap-2 shadow-xs"
              >
              ...
              <Link
                to="/sobre"
                className="px-4 py-3.5 text-text-secondary hover:text-text-primary font-semibold text-sm transition-colors"
              >
      ...
      <section id="introducao" ref={introRef} className="w-full px-6 sm:px-10 lg:px-12 pt-16 sm:pt-24 pb-20 max-w-[1600px] mx-auto">
        <div className="flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-2 max-w-4xl lg:max-w-5xl">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-text-primary leading-tight">
              O que é o Painel Territorial?
            </h2>

            <span className="text-xs font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400 mt-1">
              Sobre a Iniciativa · SECTI Bahia
            </span>

            <p className="text-sm sm:text-base text-text-primary font-normal leading-relaxed mt-2">
          ...
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400 block mb-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
              {[
                ...
                <div key={idx} className="p-4 sm:p-5 rounded-xl bg-surface border border-border flex flex-col justify-between hover:border-border-strong transition-colors shadow-2xs">
                  <h3 className="font-semibold text-sm sm:text-base text-text-primary mb-1.5">
                    {obj.t}
                  </h3>
      ...
      <section id="modulos" className="w-full px-6 sm:px-10 lg:px-12 py-16 max-w-[1600px] mx-auto border-t border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-5 sm:gap-6">
            {modulosPlataforma.map((modulo, idx) => {
              ...
              return (
                ...
                >
                  <div>
                    <span className="text-[11px] font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400 block mb-1">
                      {modulo.tag}
                    </span>
                    ...
                    <p className="text-xs sm:text-sm text-text-secondary font-normal leading-relaxed mb-4">
                      {modulo.descricao}
                    </p>

                    <div className="space-y-1.5 border-t border-border pt-3.5 mb-5">
                      <span className="text-[11px] font-semibold text-text-primary block mb-1">
                        O que compõe:
                      </span>
                      {modulo.destaques.map((item, dIdx) => (
                        <div key={dIdx} className="flex items-start gap-2 text-xs text-text-secondary leading-relaxed">
                          <span className="text-primary-600 dark:text-primary-400 font-bold shrink-0">•</span>
                          <span>{item}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <Link
                    to={modulo.rota}
                    className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors pt-3 border-t border-border"
                  >
      ...
      <section id="indicadores" className="w-full px-6 sm:px-10 lg:px-12 pt-16 pb-24 max-w-[1600px] mx-auto border-t border-border">
        <div className="flex flex-col gap-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
            {indicadoresEstrategicos.map((ind, idx) => (
              ...
              >
                <div>
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400 block mb-1">
                    {ind.tag}
                  </span>
                  <h3 className="font-semibold text-sm sm:text-base text-text-primary mb-2">
                    {ind.titulo}
                  </h3>
                  <p className="text-xs text-text-secondary leading-relaxed font-normal">
                    {ind.descricao}

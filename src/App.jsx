                      ...
                      <div>
                          ...
                          <div className="flex flex-col gap-1.5">
                              {['APL', 'IG'].map(tipo => {
                                  ...
                                  return (
                                      <button key={tipo} onClick={() => handleCadeiaProdutivaToggle(tipo)} className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all flex items-center justify-between gap-2 border ${isSelected ? (darkMode ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' : 'bg-emerald-100 border-emerald-200 text-emerald-700') : (darkMode ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50')}`}>
                                          <span>{tipo}</span>
                                      </button>
                                  )
                              })}
            ...
            <div className="animate-soft-fade relative p-2 lg:p-0 w-[96%] max-w-[1600px] mx-auto min-h-full">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                        {[
                            { l: 'Capacidade CTI', v: dashboardData.topKpis.capacidadeCti, pct: dashboardData.topKpisPct.cti, c: darkMode ? 'text-blue-400' : 'text-blue-600', b: 'bg-blue-500', sourceText: 'INEP / Censo da Educação Superior (2022)', sourceLink: 'https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/microdados/censo-da-educacao-superior', listType: 'cti' },
                            ...
                            { l: 'Cadeias Produtivas', v: dashboardData.topKpis.cadeiasIgs, pct: dashboardData.topKpisPct.cadeias, c: darkMode ? 'text-emerald-400' : 'text-emerald-600', b: 'bg-emerald-500', tr: true, sourceText: 'DataSebrae / Indicações Geográficas', sourceLink: 'https://datasebrae.com.br/indicacoesgeograficas/', listType: 'cadeias' },
                        ].map((k, idx) => (
                            <div 
                                key={idx} 
                                onClick={() => k.listType && setExpandedLists([k.listType])}
                                className={`relative p-4 rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:z-30 ${k.listType ? 'cursor-pointer' : ''} ${themeClasses.cardHover} ${darkMode ? 'bg-slate-800/40 border-slate-700/50' : 'bg-white border-slate-200/60'}`}
                            >
                                    ...
                                    {k.sourceText && (
                                        <div className="relative group flex items-center justify-center">
                                            ...
                                            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-max max-w-xs opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                                                <div className={`p-2 rounded-lg text-[10px] leading-snug shadow-lg border ${darkMode ? 'bg-slate-900 text-slate-200 border-slate-700' : 'bg-white text-slate-700 border-slate-200'}`}>
                ...
                <div className="grid grid-cols-4 lg:grid-cols-8 gap-2 lg:gap-3">
                    {[
                        ...
                        { id: 'campiUniversidadePrivada', l: 'Campi Univ. Privados', v: dashboardData.subKpis.campiUniversidadePrivada || 0, pct: (dashboardData.unfiltSubKpis.campiUniversidadePrivada || 0) > 0 ? ((dashboardData.subKpis.campiUniversidadePrivada || 0) / (dashboardData.unfiltSubKpis.campiUniversidadePrivada || 1))*100 : 0, c: darkMode ? 'text-sky-400' : 'text-sky-600', b: 'bg-sky-500', h: darkMode ? 'hover:border-sky-400' : 'hover:border-sky-600' },
                            ...
                            <div className={`absolute top-5 left-5 backdrop-blur-md px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest z-10 flex items-center gap-2.5 border shadow-lg ${darkMode ? 'bg-slate-800/80 text-white border-slate-600' : 'bg-white/90 text-slate-800 border-slate-200'}`}>
                                ...
                                <div className="relative group flex items-center justify-center">
                                    ...
                                    <div className="absolute left-1/2 -translate-x-1/2 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                                        <div className={`w-max p-2 rounded-lg text-[10px] leading-snug shadow-lg border ${darkMode ? 'bg-slate-800 text-slate-300 border-slate-600' : 'bg-white text-slate-600 border-slate-200'}`}>
